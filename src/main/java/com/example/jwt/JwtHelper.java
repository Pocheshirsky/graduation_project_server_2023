...
public class JwtHelper {
    ...
    public String generateRefreshToken(User user, RefreshToken refreshToken) {
        return JWT.create()
                .withIssuer(issuer)
                .withSubject(user.getUuid().toString())
                .withClaim("tokenId", refreshToken.getUuid().toString())
                .withIssuedAt(new Date())
    ...
    public UUID getUserIdFromRefreshToken(String token) {
        return UUID.fromString(decodeRefreshToken(token).get().getSubject());
    }

    public UUID getTokenIdFromRefreshToken(String token) {
        return UUID.fromString(decodeRefreshToken(token).get().getClaim("tokenId").asString());
    }
}
